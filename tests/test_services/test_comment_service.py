...
def test_create_reply_logic(db_session):
    ...
    CommentService.create_reply(parent.comment_id, author.account_id, "Reply")
    db_session.commit()
...
def test_comment_flattening_logic(db_session):
    ...
    db_session.add(article)
    db_session.commit()
    CommentService.create_comment(article.article_id, author.account_id, "Root")
    db_session.commit()
    root = db_session.query(Comment).filter_by(comment_content="Root").first()
    CommentService.create_reply(root.comment_id, author.account_id, "Reply A")
    db_session.commit()
    reply_a = db_session.query(Comment).filter_by(comment_content="Reply A").first()
    CommentService.create_reply(reply_a.comment_id, author.account_id, "Reply B")
    db_session.commit()
...
def test_delete_comment_as_admin(db_session):
    ...
    db_session.commit()
    result = CommentService.delete_comment(comment.comment_id, "admin")
    db_session.commit()
...
def test_create_comment_invalid_article(db_session):
    ...
    db_session.add(author)
    db_session.commit()
    result = CommentService.create_comment(999, author.account_id, "Hello")
